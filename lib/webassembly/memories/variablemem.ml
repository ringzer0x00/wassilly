...
module VariableMem = struct
  ...
  let widen (vm1 : t) (vm2 : t) : t =
    if vm1.glob = vm2.glob && vm1.loc = vm2.loc then
  ...
  let leq vm1 vm2 =
    if vm1.glob = vm2.glob && vm1.loc = vm2.loc then AD.leq vm1.ad vm2.ad
    else failwith "not compatible"

  let eq vm1 vm2 =
    if vm1.glob = vm2.glob && vm1.loc = vm2.loc then AD.eq vm1.ad vm2.ad
    else failwith "not compatible"
