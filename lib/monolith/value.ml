...
let _lub (i1 : value) (i2 : value) : value =
  Format.print_string "VALUE JOIN:\n";
  Apron.Interval.print Format.std_formatter i1;
  Apron.Interval.print Format.std_formatter i2;
  Format.print_string "\n\n RESULT (join):";
  let r =
    match (Apron.Interval.is_bottom i1, Apron.Interval.is_bottom i2) with
    | true, true -> bot
    | true, false -> i2
    | false, true -> i1
    | false, false -> (
        match Apron.Interval.cmp i1 i2 with
        | 0 -> i1
        | -1 -> i2
        | 1 -> i1
        | -2 -> Apron.Interval.of_scalar i1.inf i2.sup
        | 2 -> Apron.Interval.of_scalar i2.inf i1.sup
        | _ -> failwith "nope, can never happen valuejoin")
  in
