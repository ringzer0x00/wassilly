...
let body =
  Language.If
    ( Language.Eq (Var "loc", Num 0),
      Num 1,
      Language.Mul
        (Var "loc", Language.Call ("fact", [ Sub (Var "loc", Num 1) ])) )

let env =
  let e =
    Memory.Memory.add Eval.mgr "loc"
      (Value.makeval (Apron.Scalar.of_int 0) Value.pinf)
      (Memory.Memory.make_empty Eval.mgr)
  in
  Apron.Abstract1.print Format.std_formatter e;
  e

let call = (env, Language.Call ("fact", [ Var "loc" ]))
let funs = Funcs.add "fact" ([ "loc" ], body) Funcs.empty
let output = Value.top
